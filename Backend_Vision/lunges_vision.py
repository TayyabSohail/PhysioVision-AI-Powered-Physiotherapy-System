...
class LungesAnalyzer:
    def __init__(self, exercise="straight_leg_raises_rehab", delay_seconds=8, target_reps = 8, fps=30):
        ...
        self.peak_left_leg_angle = 180
        self.peak_right_leg_angle = 180
        self.shallow_rep_detected = False
    ...
    def check_lunges_rehab(self, landmarks):
        """Analyze rehab straight leg raises and return top 3 errors."""
        errors = []
        ...
        if right_leg_angle > 110 and self.is_above_30:
            errors.append("Bend your right leg more")
        
        if left_leg_angle > 110 and self.is_above_30:
            errors.append("Bend your left leg more")

        if right_leg_angle < 85:
            errors.append("Right leg bent too far")
        if left_leg_angle < 85:
            errors.append("Left leg bent too far")
            
        if self.is_above_30:
            self.peak_left_leg_angle = min(self.peak_left_leg_angle, left_leg_angle)
            self.peak_right_leg_angle = min(self.peak_right_leg_angle, right_leg_angle)

        if self.shallow_rep_detected == True:
            errors.append("Shallow rep, bend leg more next time")
        
        # Rep counting logic
        if right_leg_angle < 120 and not self.is_above_30:
            # Leg has just gone above 30 degrees
            self.is_above_30 = True
            self.shallow_rep_detected = False
        elif right_leg_angle > 120 and self.is_above_30:
            # Leg has come back down from above 30
            self.is_above_30 = False
            self.reps += 1  # Increment rep count

            if self.peak_left_leg_angle > 120 and self.peak_right_leg_angle > 120:
                self.shallow_rep_detected = True
                self.reps -= 1
        ...
        return errors[:3], right_leg_angle
    ...
    async def process_video(self, frame):
        ...
        if results.pose_landmarks:
            ...
            if self.frame_count <= self.delay_frames:
                ...
                errors, affected_torso_angle = self.check_lunges_rehab(results.pose_landmarks.landmark)
